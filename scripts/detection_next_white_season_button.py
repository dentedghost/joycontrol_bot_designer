...
def script():
    ...
    match = np.array(Image.open('scripts/images/white_season_button.png').convert('RGB')).ravel()
    ...
    while True:
        ...
        left_corner = IMAGE_LEFT + 525
        top_corner = IMAGE_TOP + 420
        im_current = pyautogui.screenshot('scripts/images/current_white_season_button.png',
                                          region=(left_corner, top_corner, 25, 25))
        current = np.array(Image.open('scripts/images/current_white_season_button.png').convert('RGB')).ravel()
