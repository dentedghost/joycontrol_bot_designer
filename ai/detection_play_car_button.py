...
while True:
    im_current = pyautogui.screenshot('current_play_car_button.png', region=(3493, 460, 40, 40))

    # Load images, convert to RGB, then to numpy arrays and ravel into long, flat things
    current = np.array(Image.open('current_play_car_button.png').convert('RGB')).ravel()
    match = np.array(Image.open('play_car_button.png').convert('RGB')).ravel()
